...
export function upsertEmployee(db, row, departmentId) {
  const info = db.prepare(
    `INSERT INTO employees (employee_ext_id, name, department_id, job_title, manager, employee_type, employment_status, start_date, end_date, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
     ON CONFLICT(workspace_id, employee_ext_id) DO UPDATE SET
       ...
       end_date=COALESCE(excluded.end_date, employees.end_date), updated_at=datetime('now')`
  ).run(row.employee_id, row.name, departmentId, row.job_title, row.manager, row.employee_type, row.employment_status, row.start_date || null, row.end_date || null);
